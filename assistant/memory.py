...
def clear_response_feedback():
    ...
    removed_count = len(feedback_items)
...
def dismiss_reminder_notification(identifier):
    data = load_memory()
    reminders = data.get("reminders", [])
    ...
    identifiers = normalize_reminder_text(identifier)
    ...
    key = make_reminder_notification_key(index + 1, reminder)
...
def preview_app_registry_backup_cleanup(keep_latest=5):
    memory = load_memory()
    ...
    remove_candidates = backups[:-keep_latest]
...
def update_search_folder(name, path):
    memory = load_memory()
    clean_name = normalize_entity_name(name)
