...
from assistant.trainer import (
    save_feedback, 
    find_best_match, 
    tokenize, 
    predict_intent_with_model, 
    evaluate_model, 
    summarize_confusion, 
    get_debug_weights, 
    similarity_score, 
    get_dataset_stats, 
    get_dataset_intent_counts, 
    get_dataset_duplicates, 
    get_dataset_conflicts, 
    get_dataset_broken_records, 
    get_dataset_missing_fields, 
    get_dataset_unknown_intents, 
    get_dataset_coverage, 
    get_dataset_low_coverage, 
    get_dataset_suggestions, 
    get_dataset_examples_for_intent, 
    get_dataset_intent_health, 
    get_dataset_weakest_intent, 
    get_dataset_strongest_intent, 
    get_dataset_balance, 
    suggest_example_phrases_for_intent, 
    get_conflicts_for_intent, 
...
def parse_delete_note_memory_result_index(user_input):
    text = user_input.lower().strip()
    
    for prefix in ["delete note memory result ", "remove note memory result "]:
        if text.startswith(prefix):
            value = text.replace(prefix, "", 1).strip()
...
def parse_resolver_conflict(user_input):
    text = user_input.lower().strip()
...
def parse_register_app(user_input):
    return apps.parse_register_app(user_input)

def parse_app_alias(user_input):
    return apps.parse_app_alias(user_input)

def parse_remove_app_alias(user_input):
    return apps.parse_remove_app_alias(user_input)

def parse_update_app(user_input):
    return apps.parse_update_app(user_input)

def parse_default_app(user_input):
    return apps.parse_default_app(user_input)

def parse_debug_app(user_input):
    return apps.parse_debug_app(user_input)

def parse_remove_default_app(user_input):
    return apps.parse_remove_default_app(user_input)

def parse_delete_app_registry_index(user_input):
    return apps.parse_delete_app_registry_index(user_input)

def parse_backup_index(user_input, prefix):
    return apps.parse_backup_index(user_input, prefix)

def resolve_app_name_for_open(app_name):
    return apps.resolve_app_name_for_open(app_name)
    
def parse_rename_app_registry_index(user_input):
    return apps.parse_rename_app_registry_index(user_input)

def parse_unregister_app(user_input):
    return apps.parse_unregister_app(user_input)

def parse_app_search(user_input):
    return apps.parse_app_search(user_input)

def parse_allow_app(user_input):
    return apps.parse_allow_app(user_input)

def parse_disallow_app(user_input):
    return apps.parse_disallow_app(user_input)
...
def format_focus_session_summary(task, duration, notes):
    return focus.format_focus_session_summary(task, duration, notes)
...
def stop_focus_mode():
    return focus.stop_focus_mode()
    
def get_focus_mode():
    return focus.get_focus_mode()

def get_focus_task():
    return focus.get_focus_task()

def get_focus_session_dates():
    return focus.get_focus_session_dates()

def save_focus_session(task, started_at):
    return focus.save_focus_session(task, started_at)

def get_focus_goal_progress_summary():
    return focus.get_focus_goal_progress_summary()
...
def build_today_focus_stats():
    return focus.build_today_focus_stats()
    
def build_recent_focus_stats(days):
    return focus.build_recent_focus_stats(days)
    
def build_best_focus_day():
    return focus.build_best_focus_day()
    
def build_true_focus_streak():
    return focus.build_true_focus_streak()
...
def choose_focus_task():
    return focus.choose_focus_task()

def add_current_focus_note(note):
    return focus.add_current_focus_note(note)
    
def get_current_focus_notes():
    return focus.get_current_focus_notes()
...
def parse_memory_query_with_type(text):
    ...
    if parts and parts[-1].lower() in ["noarchive", "archiveonly"]:
        archive_mode = parts[-1].lower()
        parts = parts[:-1]
    ...
    if parts:
        ...
        if possible_type in MEMORY_TYPE_ALIASES:
            memory_type = MEMORY_TYPE_ALIASES[possible_type]
            parts = parts[:-1]
            
    text = " ".join(parts).strip()
            
    return text, memory_type, limit, min_score, sort_mode, date_filter, date_value, date_start, date_end, archive_mode

def semantic_search_memory(query, limit=5, min_score=0.3, source_type=None, memory_type=None, sort_mode="relevant",date_filter=None,date_value=None,date_start=None,date_end=None, archive_mode="include"):
    ...
    for item in items:
        ...
        if archive_mode == "noarchive" and item["type"] == "archive_summary":
            continue
        
        if archive_mode == "archiveonly" and item["type"] != "archive_summary":
            continue
...
def debug_memory_search(query, limit=5, memory_type=None, min_score=0.0, sort_mode="relevant", date_filter=None, date_value=None, date_start=None, date_end=None, archive_mode="include"):
    items = collect_memory_search_items()
    debug_results = []
...
def format_duration_since(timestamp):
    return focus.format_duration_since(timestamp)

def format_duration_from_seconds(total_seconds):
    return focus.format_duration_from_seconds(total_seconds)

def calculate_duration_seconds(started_at, ended_at):
    return focus.calculate_duration_seconds(started_at, ended_at)
...
def handle_control_intent(user_input, analysis):
    ...
    if intent == "debug_memory_search":
        ...
        results = debug_memory_search(query, limit=limit, memory_type=memory_type, min_score=min_score, sort_mode=sort_mode,date_filter=date_filter, date_value=date_value, date_start=date_start, date_end=date_end, archive_mode=archive_mode)
    ...
    if intent == "debug_app_resolution":
        return apps.format_debug_app_resolution(user_input)
    ...
    if intent == "debug_focus_goal":
        ...
        all_stats = focus.build_focus_stats()
    ...
    if intent == "debug_app_cleanup":
        return apps.format_debug_app_cleanup()
    ...
    if intent == "dataset_duplicates":
        duplicates = get_dataset_duplicates()
        
        if not duplicates:
            return "I did not find duplicate dataset examples."
        
        lines = ["Duplicate dataset examples:"]
        
        for item in duplicates[:10]:
            lines.append(
                f"- {item['input']} | "
                f"{item['first_dataset']}:{item['first_intent']} -> "
                f"{item['duplicate_dataset']}:{item['duplicate_intent']}"
            )
    ...
    if intent == "dataset_broken_records":
        records = get_dataset_broken_records()
        
        if not records:
            return "I did not find broken dataset records."
        
        lines = ["Broken dataset records:"]
        
        for item in records[:10]:
            lines.append(
                f"- {item['dataset']} line {item['line']}: {item['error']}"
            )
    ...
    if intent == "dataset_unknown_intents":
        unknown = get_dataset_unknown_intents(VALID_INTENTS)
        
        if not unknown:
            return "I did not find dataset records with unknown intents."
        
        lines = ["Dataset unknown intents:"]
        
        for item in unknown[:10]:
            lines.append(
                f"- {item['dataset']} record {item['record']}: "
                f"{item['intent']} | {item['input']}"
            )
    ...
    if intent == "dataset_suggestions":
        suggestions = get_dataset_suggestions(VALID_INTENTS)
        
        if not suggestions:
            return "I do not have dataset suggestions right now."
        
        lines = ["Dataset suggestions:"]
    ...
    if intent == "dataset_strongest_intent":
        strongest = get_dataset_strongest_intent(VALID_INTENTS)
        
        if not strongest:
            return "I could not find a strongest intent."
        
        return (
            f"Strongest dataset intent:\n"
            f"Intent: {strongest['intent']}\n"
            f"Examples: {strongest['count']}"
        )
        
    if intent == "dataset_balance":
        balance = get_dataset_balance(VALID_INTENTS)
        
        if not balance:
            return "I could not calculate dataset balance."
        
        return (
            f"Dataset balance:\n"
            f"Weakest: {balance['weakest']['intent']} ({balance['weakest']['count']})\n"
    ...
    if intent == "dataset_suggest_examples_for_intent":
        ...
        if conflicts:
            lines.append(f"Warning: this intent has {len(conflicts)} dataset conflict(s).")
            lines.append("Fix conflicts before adding too many more examples.")
    ...
    if intent == "dataset_conflict_summary":
        summary = get_dataset_conflict_summary()
        
        if not summary:
            return "I did not find dataset conflicts to summarize."
        
        lines = ["Dataset conflict summary:"]
    ...
    if intent == "resolve_dataset_conflict":
        parsed = parse_resolver_conflict(user_input)
        
        if not parsed:
            return "Use this format: resolve conflict input text as intent_name"
        ...
        result = resolve_dataset_conflict(input_text, target_intent)
        
        if not result["changed"] and not result["unchanged"]:
            return f"I did not find dataset records for: {input_text}"
        
        lines = [f"Dataset conflict resolved for '{input_text}' -> {target_intent}:"]
        
        for item in result["changed"]:
            lines.append(
                f"- changed {item['dataset']} record {item['record']}: "
                f"{item['old_intent']} -> {item['new_intent']}"
            )
            
        for item in result["unchanged"]:
            lines.append(
                f"- kept {item['dataset']} record {item['record']}: {item['reason']}"
            )
    ...
    if intent == "backup_datasets":
        ...
        if not backups:
            return "I did not find dataset files to back up."
        
        lines = ["Dataset backups created:"]
    ...
    if intent == "restore_dataset_backup":
        index = parse_dataset_backup_index(user_input, "restore dataset backup ")
        
        if index is None:
            return "Use this format: restore dataset backup number"
        
        result = restore_dataset_backup(index)
        
        if not result["restored"]:
            return f"I could not restore dataset backup: {result['reason']}"
        
        return (
            f"Dataset backup restored.\n"
            f"Backup: {result['backup']['name']}\n"
            f"Dataset: {result['dataset']}\n"
            f"Restored to: {result['target']}"
        )
    ...
    if intent == "show_dataset_backups":
        ...
        if not backups:
            return "I do not have dataset backups yet."
        
        lines = ["Dataset backups:"]
        
        for item in backups:
            lines.append(
                f"- {item['name']} | {item['size']} bytes | "
                f"{format_timestamp(item['modified'])}"
            )
            
        return "\n".join(lines)
    
    if intent == "cleanup_dataset_backups":
        result = cleanup_dataset_backups()
        
        return (
            f"Dataset backup cleanup finished.\n"
            f"Total backups before cleanup: {result['total']}\n"
            f"Kept latest: {result['keep_latest']}\n"
...
def handle_memory_intent(user_input, analysis):
    ...
    if intent == "work_session_status":
        is_active = get_focus_mode()
        current_task = get_focus_task()
        pending_task = get_pending_task()
        ...
        if pending_task:
            return (
                "Work session is paused.\n"
                f"Pending focus: {pending_task}\n"
                "Use: resume work session"
            )
            
        suggested_task = choose_focus_task()
        
        if suggested_task:
            return (
                "No work session is active.\n"
                f"Suggested focus: {suggested_task}\n"
                "Use: start work session"
            )
            
        return "No work session is active, and I do not have a suggested task right now."
    
    if intent == "work_session_plan":
        pending_task = get_pending_task()
        suggested_task = choose_focus_task()
        ...
        if get_focus_mode():
            ...
            return (
                "Current work plan:\n"
                f"1. Continue focusing on: {current_task}\n"
                "2. Check progress when ready.\n"
                "3. End the work session when finished."
            )
            
        if pending_task:
            return (
                "Current work plan:\n"
                f"1. Resume paused task: {pending_task}\n"
                "2. Work on it for one focus session.\n"
                "3. End or pause when needed."
            )
            
        if suggested_task:
            return (
                "Suggested work plan:\n"
                f"1. Start with: {suggested_task}\n"
                "2. Focus on one small step.\n"
                "3. Review progress after the session."
            )
    ...
    if intent == "work_session_summary":
        summary = memory.get_work_session_summary()
        
        if summary["total_sessions"] == 0:
            return "I do not have any completed work sessions to summarize yet."
        
        total_duration = focus.format_duration_from_seconds(summary["total_seconds"])
    ...
    if intent == "work_session_task_breakdown":
        breakdown = memory.get_work_session_task_breakdown()
        
        if not breakdown:
            return "I do not have any completed work sessions to break down yet."
        
        lines = ["Work session task breakdown:"]
    ...
    if intent == "today_work_review":
        review = memory.get_today_work_review()
        
        if review["total_sessions"] == 0:
            return f"I do not have any completed work sessions for today ({review['date']}) yet."
        
        total_duration = focus.format_duration_from_seconds(review["total_seconds"])
        
        lines = [
            f"Today work review: {review['date']}",
    ...
    if intent == "semantic_memory_search":
        ...
        results = semantic_search_memory(query, memory_type=memory_type, limit=limit, min_score=min_score, sort_mode=sort_mode, date_filter=date_filter, date_value=date_value, date_start=date_start, date_end=date_end, archive_mode=archive_mode)
    ...
    if intent == "delete_note_memory_result":
        ...
        if index is None:
            return "Use this format: delete note memory result number"
    ...
    if intent == "focus_dashboard":
        today_stats = build_today_focus_stats()
        weekly_stats = build_recent_focus_stats(7)
        streak = build_true_focus_streak()
        focus_progress = get_focus_goal_progress_summary()
        
        lines = ["Focus dashboard:"]
        ...
        lines.append(
            f"Today: {today_stats['total_sessions']} session(s), "
            f"{format_duration_from_seconds(today_stats['total_seconds'])}"
        )
        
        lines.append(
            f"Week: {weekly_stats['total_sessions']} session(s), "
            f"{format_duration_from_seconds(weekly_stats['total_seconds'])}"
        )
        
        if focus_progress:
            lines.append(f"Goal: {focus_progress['goal']}")
            
            if focus_progress["progress"] is not None:
                lines.append(f"Goal progress: {focus_progress['progress']}%")
            else:
                lines.append("Goal progress: unknown")
    ...
    if intent == "register_app":
        return apps.handle_register_app(user_input)
    
    if intent == "set_default_app":
        return apps.handle_set_default_app(user_input)
    
    if intent == "remove_default_app":
        return apps.handle_remove_default_app(user_input)
    
    if intent == "show_default_apps":
        return apps.format_default_apps()
    
    if intent == "add_app_alias":
        return apps.handle_add_app_alias(user_input)
    
    if intent == "remove_app_alias":
        return apps.handle_remove_app_alias(user_input)
    
    if intent == "show_app_aliases":
        return apps.format_app_aliases()
    
    if intent == "update_registered_app":
        return apps.handle_update_registered_app(user_input)
    
    if intent == "unregister_app":
        return apps.handle_unregister_app(user_input)
    
    if intent == "rename_app_registry_by_index":
        return apps.handle_rename_app_registry_by_index(user_input)
    
    if intent == "delete_app_registry_by_index":
        return apps.handle_delete_app_registry_by_index(user_input)
    
    if intent == "search_app_registry":
        return apps.handle_search_app_registry(user_input)
    ...
    if intent == "allow_app":
        return apps.handle_allow_app(user_input)
    
    if intent == "disallow_app":
        return apps.handle_disallow_app(user_input)
    
    if intent == "app_safety_dashboard":
        return apps.format_app_safety_dashboard()
    
    if intent == "preview_app_cleanup":
        return apps.format_app_cleanup_preview()
        
    if intent == "repair_app_cleanup":
        return apps.handle_repair_app_cleanup()
        
    if intent == "backup_app_registry":
        return apps.handle_backup_app_registry(current_timestamp())
        
    if intent == "show_app_registry_backups":
        return apps.format_app_registry_backups()
    
    if intent == "preview_restore_app_registry_backup":
        return apps.format_restore_app_registry_preview(user_input)
        
    if intent == "preview_app_backup_cleanup":
        return apps.format_app_backup_cleanup_preview()
        
    if intent == "restore_app_registry_backup":
        return apps.handle_restore_app_registry_backup(user_input)
    ...
    if intent == "search_text_files":
        ...
        parsed = parse_text_search_command(user_input)
...
def handle_chat_intent(user_input, analysis):
    ...
    handled = chat.handle_chat_intent(user_input, analysis, memory, personality, current_timestamp)
...
def get_response(user_input):
    ...
    if analysis["intent"] not in ["chat_explain_last", "chat_repeat_last"]:
        ...
        memory.set_state_value("last_response_text", response)
